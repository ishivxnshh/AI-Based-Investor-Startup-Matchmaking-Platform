...
// Optimized compound indexes for findForStartup and findForInvestor
matchSchema.index({ startup: 1, isActive: 1, status: 1, matchScore: -1 });
matchSchema.index({ investor: 1, isActive: 1, status: 1, matchScore: -1 });
...
matchSchema.statics.countUnreadForUser = async function (profileId, userId, accountType) {
  const matchQuery = { isActive: true };
  if (accountType === 'startup') {
    matchQuery.startup = profileId;
  } else {
    matchQuery.investor = profileId;
  }
  ...
  const result = await this.aggregate([
    ...
    {
      $match: {
        'communication.sender': { $ne: userId },
        'communication.readBy.user': { $ne: userId }
      }
    },
    { $count: 'unreadCount' }
  ]);
